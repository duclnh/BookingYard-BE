...
{
	public class GetAllBookingAdminQueryHandler : IRequestHandler<GetAllBookingAdminQuery, PagingResult<BookingDetailDto>>
	{
		private readonly IBookingRepository _bookingRepository;
		private readonly IMapper _mapper;

		public GetAllBookingAdminQueryHandler(IBookingRepository bookingRepository, IMapper mapper)
		{
			_bookingRepository = bookingRepository;
			_mapper = mapper;
		}

		public async Task<PagingResult<BookingDetailDto>> Handle(GetAllBookingAdminQuery request, CancellationToken cancellationToken)
		{
			...
			if (!string.IsNullOrEmpty(request.requestParams.Search))
			{
				...
				expressions.Add(x => x.Court.Facility.Name.ToLower().Contains(search) || x.FullName.ToLower().Contains(search));
